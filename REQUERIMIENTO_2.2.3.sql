...
FROM POSTULANTE P
                ...
                JOIN COMUNA CO ON P.ID_COMUNA = CO.ID_COMUNA AND P.ID_REGION = CO.ID_REGION
                JOIN REGION R ON CO.ID_REGION = R.ID_REGION
                JOIN RANGO_NOTAS RN ON P.NOTA_PREGRADO BETWEEN RN.NOTA_MIN AND RN.NOTA_MAX
                JOIN EXPERIENCIA_LABORAL EL ON P.NUMRUN = EL.NUMRUN
                JOIN FORMULARIO_POSTULACION F ON P.NUMRUN = F.NUMRUN
                JOIN DOCENCIA_INVEST D ON F.ID_DOCENCIA_INV = D.ID_DOCENCIA_INV
                JOIN OBJETIVO_ESTUDIO OE ON F.ID_OBJETIVO = OE.ID_OBJETIVO
                JOIN DECLARACION_INTERESES DI ON F.ID_INTERESES = DI.ID_INTERESES
                JOIN RETRIBUCION RE ON F.ID_RETRIBUCION = RE.ID_RETRIBUCION
                JOIN INSTITUCION_ACADEMICA INST ON F.ID_INSTITUCION = INST.ID_INSTITUCION
                JOIN RANGO_PTJE_RANKING RPR ON INST.RANKING_INST BETWEEN RPR.RANKING_MIN AND RPR.RANKING_MAX)
